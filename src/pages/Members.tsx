    ...
    <div className="p-4 sm:p-8 max-w-5xl space-y-6">
              ...
              <div key={m.id} className="px-5 py-4 flex items-center gap-3 flex-wrap sm:flex-nowrap">
                ...
                <div className="flex gap-2 shrink-0 flex-wrap">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs bg-emerald-600 text-white hover:bg-emerald-700 rounded-md flex items-center gap-1"
                  >
                    <UserCheck className="w-3.5 h-3.5" /> Approve
                  </button>
                  <button
                    onClick={() => handleRemove(m)}
                    className="px-3 py-1.5 text-xs bg-stone-100 text-stone-700 hover:bg-stone-200 rounded-md flex items-center gap-1"
                  >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {org.inviteCode && (
                <button
                  ...
                  className="px-3 py-1.5 text-xs border border-stone-300 text-stone-700 hover:bg-stone-100 rounded-md flex items-center gap-1.5"
                >
      ...
      >
        {loading ? (
          ...
          <>
            {/* Mobile: card list */}
            <div className="sm:hidden -mx-5 -mb-5 divide-y divide-stone-100">
                ...
                <div key={m.id} className="px-5 py-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        ...
                        {isAdmin && m.id !== user.uid ? (
                          <select
                            ...
                            className="text-xs border border-stone-200 rounded px-1.5 py-1 bg-white text-stone-700 focus:outline-none shrink-0"
                          >
                      ...
                      <div className="text-xs text-stone-400 mb-2">{m.email}</div>
                      <div className="flex flex-wrap items-center gap-2">
                        {isAdmin && m.id !== user.uid ? (
                          <select
                            value={m.role}
                            onChange={(e) => handleRoleChange(m, e.target.value)}
                            className="text-xs border border-stone-200 rounded px-1.5 py-1 bg-white text-stone-700 focus:outline-none"
                          >
                          ...
                          <span className="text-xs text-stone-600">{ROLE_LABELS[m.role as Role] ?? m.role}</span>
                        )}
                        {isAdmin ? (
                          <button
                        ...
                        {isAdmin && m.id !== user.uid && (
                          <button
                            onClick={() => handleRemove(m)}
                            className="text-stone-300 hover:text-red-500 ml-auto p-1"
                            title="Remove member"
                          >
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between pt-2 border-t border-stone-100">
            ...
            <button
              ...
              className="px-4 py-2 bg-stone-900 text-stone-50 text-sm font-medium rounded-md hover:bg-stone-800 disabled:opacity-40 flex items-center gap-2"
            >
